class Operation:
    ...
    def xor_block(cls, first_in, second_in):
    ...
    def times_three(cls, input, block_size):
        if len(input) != block_size:
            raise ValueError("Input must have same size as block size")
        output = cls.times_two(input, 8)
        output = cls._xor_block(output, input)
        return output
        

    def times_two(cls, input, block_size):
        if len(input) != block_size:
            raise ValueError("Input must have same size as block size")
